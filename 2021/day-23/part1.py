...
def dump(state, length=2):
    ...
    for i in range(length - 1, -1, -1):
        ...
        print(
            px,
            "#",
            state[1][i] if len(state[1]) > i else ".",
            "#",
...
def possible_moves(state, length=2):
    ...
    for hall_pos, hall_el in enumerate(state[0]):
        if hall_el == ".":
            continue
        if check_room(hall_el, state) and check_path(hall_pos, ROOMS[hall_el], state):
            new_state = state
            new_state = tuple_replace(
                tuple_replace(".", hall_pos, state[0]), 0, new_state
            )
            new_state = tuple_replace(
            ...
            new_cost = (
                abs(hall_pos - ROOMS[hall_el])
                + (length - len(state[ROOMS_IDX[hall_el]]))
            ) * COST[hall_el]
    ...
    for room, room_pos in ROOMS.items():
        ...
        for hall_pos, hall_el in enumerate(state[0]):
            ...
            new_state = state
            new_state = tuple_replace(
                tuple_replace(tmp_el, hall_pos, state[0]), 0, new_state
            )
            new_state = tuple_replace(tmp_room, ROOMS_IDX[room], new_state)
            new_cost = (
                abs(hall_pos - room_pos) + (length - len(new_state[ROOMS_IDX[room]]))
            ) * COST[tmp_el]
            yield (new_state, new_cost)
...
for part in PARTS:
    ...
    while len(queue) > 0:
        ...
        if check_winner(q_state):
            ...
            continue
        if q_state in visited and (visited[q_state] <= q_sum):
            continue
        visited[q_state] = q_sum
        for loop_state, loop_sum in possible_moves(
            q_state, length=2 if part == 1 else 4
        ):
