...
def reverse_rotate(rotation):
    tmp = [["x", rotation[0]], ["y", rotation[1]], ["z", rotation[2]]]
    result = {}
    for pair in tmp:
        if pair[1][0] == "+":
            pair[0] = "+" + pair[0]
...
def rotate_scanner(sensor, rotation):
    return [rotate_point(point, rotation) for point in sensor]


def normalize_point(point, ref_point, d=1):
    return (
        point[0] - d * ref_point[0],
...
def shift_scanner(sensor, rotation, ref_point0, ref_point1):
    return normalize_scanner(
        normalize_scanner(rotate_scanner(sensor, rotation), ref_point1), ref_point0, -1
    )
...
def detect_scanners(current, rotations, pairs_matrix, scanners_list, coordinates):
...
with open("../../_inputs/2021/day-19/input.txt", "r", encoding="utf-8") as f:
    ...
    for line in f:
        ...
        input_data[i].append((int(x), int(y), int(z)))


scanner_pairs = [[(False, None, None, None) for _ in input_data] for _ in input_data]
    ...
    for j, scanner1 in input_data.items():
        ...
        if check_result[0] is True:
            ...
            scanner_pairs[j][i] = (
                ...
                rotate_point(check_result[3], reverse_rotate(check_result[1])),
                rotate_point(check_result[2], reverse_rotate(check_result[1])),
            )
print()
...
distances = [[0 for _ in scanners_coords] for _ in scanners_coords]
for i, scanner_i in scanners_coords.items():
    for j, scanner_j in scanners_coords.items():
        if i >= j:
            continue
        distances[i][j] = (
            abs(scanner_i[0] - scanner_j[0])
