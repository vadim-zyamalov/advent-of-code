    ...
    for digit in range(start, finish, delta):
        ...
        while True:
            ...
            if com == "inp":
                ...
                break
            if com == "add":
                set_register(get_register(arg_0, state) + arg_1, arg_0, state)
            if com == "mul":
                set_register(get_register(arg_0, state) * arg_1, arg_0, state)
            if com == "div":
                set_register(get_register(arg_0, state) // arg_1, arg_0, state)
            if com == "mod":
                set_register(get_register(arg_0, state) % arg_1, arg_0, state)
            if com == "eql":
                set_register(int(get_register(arg_0, state) == arg_1), arg_0, state)
            i += 1
        ...
        if len(tmp) == 2:
            ...
            PROGRAM.append(
                (
                    tmp[0],
                    tmp[1],
                    tmp[2] if tmp[2] in ["w", "x", "y", "z"] else int(tmp[2]),
                )
            )
