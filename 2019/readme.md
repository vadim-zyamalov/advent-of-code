...
$$x_{new} = a x + b \bmod N$$
где $x, x_{new}$ --- исходный и конечный индексы, $N$ --- размер колоды, $a, b$ --- какие-то коэффициенты.
...
1. перекладывание колоды в новую стопку, как не трудно убедиться, эквивалентно отражению колоды в обратном порядке.
   ...
   Пусть текущий индекс равен $x_{now} = a x + b \bmod N$.
   Тогда можно показать, что после преобразования полином примет вид
   $$\hat{x}_{now} = -a x + (N - b - 1) \bmod N$$
   Действительно, пусть искомый полином равен $\hat{x}_{now} = \hat{a} x + \hat{b} \bmod N$.
   Также очевидно, что при инверсии $\hat{x}_{now} = N - x_{now} - 1 \bmod N,\ \forall x$.

   Тогда, взяв $x=0$, получим
...
2. перенос части колоды из начала в конец и наоборот.
   ...
   $$\hat{x}_{now} = x_{now} - n \bmod N,\ \forall x$$
   откуда при $x=0$ следует
...
3. раскладывание с шагом.
   ...
   $$\hat{x}_{new} = n x \bmod N = a n x + n b \bmod N,\ \forall x$$
   Отсюда имеем
   $$\hat{b} = n b \bmod N$$
   $$\hat{a} = n a \bmod N$$
...
$$f(x) = a x + b \bmod N$$
$$g(x) = c x + d \bmod N$$
$$f^2(x) \coloneqq f(f(x)) = a^2 x + a b + b \bmod N$$
$$g(f(x)) = a c x + c b + d \bmod N$$
