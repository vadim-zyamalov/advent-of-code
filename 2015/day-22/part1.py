...
def player_turn(player, boss, timer, spells):
    ...
    result = []
    for s in spells:
        if (player["mana"] >= spells[s]["cost"]) and (timer[s] == 0):
            ...
            tmp_result = boss_turn(
                loop_player.copy(), loop_boss.copy(), loop_timer.copy(), spells
            )
            if tmp_result:
...
# Part 1
difficulty = 0
result = player_turn(player.copy(), boss.copy(), timer.copy(), spells)
