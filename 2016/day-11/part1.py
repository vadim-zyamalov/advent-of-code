...
ELEMENTS = {"Th": 1, "Pu": 2, "Sr": 3, "Pr": 4, "Rt": 5, "El": 6, "Di": 7}
...
def remove_from_floor(element, data):
    return sort_floor(tuple(el for el in data if el not in element))
...
def possible_moves(state):
    result = []
    cur_floor = state[0]
    floor_variants = [cur_floor + d for d in [-1, 1] if 1 <= cur_floor + d <= 4]
    # Move one or two elements
    moves = list(combinations(state[cur_floor], r=1)) + list(
        combinations(state[cur_floor], r=2)
    )
    for move in moves:
...
def encode_state(state):
    return (
        ...
        sort_floor(
            tuple(
                -ELEMENTS[el[:-1]] if el[-1] == "M" else ELEMENTS[el[:-1]]
                for el in state[1]
            )
        ),
        sort_floor(
...
START = encode_state(
    (1, ("ThG", "ThM", "PuG", "SrG"), ("PuM", "SrM"), ("PrG", "PrM", "RtG", "RtM"), ())
)
